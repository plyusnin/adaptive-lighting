...
только если сначала включить лампу на минимальной яркости с `transition: 0`, а
следом отправить адаптивную цель с нужным transition.
...
каждая лампа из `off_lights` включается на `FADE_IN_BRIGHTNESS = 1` — это
**один процент**, и отправляется он именно как `brightness_pct` — с
`transition: 0` — функция `_async_fade_in_from_zero` в
[`switch.py`](custom_components/adaptive_lighting/switch.py).

**Почему процент, а не `brightness`.** `brightness: 1` — это 1 из 255, то есть
0.4 %. Интеграции с процентной шкалой на стороне устройства (Shelly и другие
драйверы, конвертирующие 0–255 в 0–100) округляют такое значение **до 0 %**, а
`brightness: 0` для лампы означает «выключено»: вместо разгорания лампа не
включается вовсе (или сразу гаснет), и адаптация после этого стартует не из нуля,
а с прежнего запомненного уровня — ровно та проблема, которую разгорание должно
решать. `brightness_pct: 1` — минимальное значение, которое переживает такое
округление на любой шкале: 1 % от 255 — это 3, а не 0. У ламп с 0–255 разница
между 1 и 3 на глаз неразличима, так что для них процент ничего не ломает.
...
Тесты: `test_apply_fades_in_off_light_from_zero` (две команды —
`(brightness_pct: 1, transition 0)`,
